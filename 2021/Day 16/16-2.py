    ...
    if typeID == 4:
        ...
        if lenType == 0:
            ...
            while not len(tempDic) == 0:
                tempDic, totVer, valInTemp = parse(tempDic, totVer)
                valIn += (valInTemp)
            arr = arr[packLen:]
        else:
            ...
            for i in range(packLen):
                arr, totVer, valInTemp = parse(arr, totVer)
                valIn += (valInTemp)
    if typeID == 0: 
        val.append(sum(valIn))
    elif typeID == 1: 
        val.append(math.prod(valIn))
    elif typeID == 2: 
        val.append(min(valIn))
    elif typeID == 3: 
        val.append(max(valIn))
    elif typeID == 5: 
        if valIn[0] > valIn[1]:
            val.append(1)
        else:
